import BinaryTreeInterface from "./BinaryTree.interface";
import { options } from "../config";

class Node implements BinaryTreeInterface {
  id: number;
  parent: BinaryTreeInterface | null;
  value: number | string | null;
  left: BinaryTreeInterface | null;
  right: BinaryTreeInterface | null;
  static counter = 0;

  constructor(value: number | null, parent: BinaryTreeInterface | null = null) {
  ...
  _convert(item: any, i = 0) {
    return isNaN(item) ? parseInt(item.charCodeAt(i), 10) : parseInt(item, 10);
  }

  _handleEqual(item: any, itemValue: number, thisValue: number) {
    if (isNaN(item)) {
      let index = 1;
      while (itemValue === thisValue) {
        ...
        if (typeof this.value !== "number" && index >= this.value!.length) {
  ...
  _compareValues(item: number, itemValue: number, thisValue: number) {
    if (itemValue < thisValue) {
      this.addLeftChild(item);
